...
function SmsCapacityProgress({ used, total }: { used?: number | null, total?: number | null }) {
  if (used === undefined || used === null || total === undefined || total === null || total <= 0) return <Typography variant="body2" sx={{ fontSize: '0.825rem' }}>N/A</Typography>;
    ...
    try {
      ...
      if (simRes.data) {
        ...
        const missingSlowFields =
          data.present && (!data.phone_numbers?.length || !data.sms_center || data.sms_total === undefined || data.sms_total === null || data.sms_total <= 0)
